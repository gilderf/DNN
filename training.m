...
if exist('method', 'var') ~= 1; method = 'SCE'; end
...
for ep=1:epoch
    for iter7=batch:batch:P
        ...
        for iter8=0:batch*ln-1 % parfor
            ...
            W(:,:,1) = propagation(W(:,:,1), z(1), k, U);
            for iter9=1:lz
                W(:,:,iter9+1) = propagation(W(:,:,iter9).*DOES(:,:,iter9), f(iter9), k, U);
            end
        ...
        % обновляем веса
        norm_tmp = max(max(max(abs(grad))));
        if(norm_tmp > 0)
            grad = speed*grad/norm_tmp;
            DOES = DOES.*exp(2i*pi*grad);
        end
...
clearvars num num2 iter7 iter8 iter9 ep epoch P speed me mi W F T argmax Accr cycle f Target ...
    randind grad batch method speeddown lz norm_tmp;
